...
class DenseRankFunction : public exec::WindowFunction {
 public:
  explicit DenseRankFunction() : WindowFunction(BIGINT(), nullptr) {}
  ...
  void apply(
    ...
    for (int i = 0; i < numRows; i++) {
      ...
      rawValues[resultOffset + i] = denseRank_;
    }
  }

 private:
  int32_t currentPeerGroupStart_ = 0;
  int64_t denseRank_ = 1;
};
...
void registerDenseRank(const std::string& name) {
  ...
  exec::registerWindowFunction(
      ...
      [name](
        ...
        return std::make_unique<DenseRankFunction>();
      });
}
} // namespace facebook::velox::window
